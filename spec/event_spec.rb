...
describe 'event' do
  ...
  describe 'more functionality' do
    before :each do
      @event = Event.new("South Pearl Street Farmers Market")
      @food_truck1 = FoodTruck.new("Rocky Mountain Pies")
      @item1 = Item.new({name: 'Peach Pie (Slice)', price: "$3.75"})
    ...
    it 'can create a sorted list of items for sale' do

      expect(@event.sorted_item_list).to eq [@item2, @item4, @item1, @item3]
    end

    it 'can generate a list of items that are overstocked' do

      expect(@event.overstocked_items).to eq [@item1]
    end

    it 'can generate a hash of all items at the event' do
